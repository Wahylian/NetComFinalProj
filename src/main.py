...
def do_all_pcap_files_exist() -> bool:
    """
    This function makes sure all the required pcap files are present in the project
    """
    # going over each pcap file
    if not path.exists("WiresharkRecordings"):
        return False
    if not path.exists("WiresharkRecordings/ChromeRecordingFiltered.pcapng"):
        print("ChromeRecordingFiltered.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/FirefoxRecordingFiltered.pcapng"):
        print("FirefoxRecordingFiltered.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/SpotifyRecordingFiltered.pcapng"):
        print("SpotifyRecordingFiltered.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/YoutubeRecordingFiltered.pcapng"):
        print("YoutubeRecordingFiltered.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/ZoomRecordingFiltered.pcapng"):
        print("ZoomRecordingFiltered.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/Q4Traffic.pcapng"):
        print("Q4Traffic.pcapng missing")
        return False
    if not path.exists("WiresharkRecordings/BonusTraffic.pcapng"):
        print("BonusTraffic.pcapng missing")
        return False

    # all pcap files exist
    return True


def do_all_csv_files_exist() -> bool:
    """
    This function makes sure all the required csv files are present in the project
    """
    # going over each csv file
    if not path.exists("WiresharkRecordingSpecialCSV"):
        print("Please add back the folder 'WiresharkRecordingSpecialCSV' to your project in the same folder"
              " as this file")
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/chrome.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/firefox.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/spotify.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/youtube.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/zoom.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/q4.csv"):
        return False
    if not path.exists("WiresharkRecordingSpecialCSV/bq.csv"):
        return False
...
def main():
    if not do_all_pcap_files_exist():
        print("Please add the files required in the 'README.MD' file before running this program")
        return
    ...
    if answer == "n" and not do_all_pcap_files_exist():
        print("The csv files required are missing, creating csv files...")
        create_csv_from_pcapngs()

    while True:
        print("Press 1 to view graphs related to questions 1-3,"
              " press 2 for graphs related to question 4, 3 for bonus question's graphs")
        print("press 0 to quit:")
        answer = str(input())

        # making sure input is correct
        while answer != '0' and answer != '1' and answer != '2' and answer != '3':
            answer = str(input("please enter 1, 2 or 3, or 0 to quit: "))
